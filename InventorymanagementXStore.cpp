...
// Order Logic for General Product Store
...
void selectPaymentMethod(string& paymentMethod, double total) {
    ...
    while (true) {
        ...
        if (paymentChoice == 1) {
            ...
            cout << "Send to store account number: 09123456789\n";
            char confirmation;
            ...
            cout << "Credit Card payment selected. Please proceed to payment terminal when you pick up your items.\n";
            break;
        } else {
...
void placeOrder() {
    ...
    while (true) {
        ...
        // After finishing this catalog, ask if want to add from another category
        cout << "Do you want to add products from another category? (Y/N): ";
        string addMore;
        getline(cin, addMore);
        if (addMore.length() && (addMore[0] == 'y' || addMore[0] == 'Y')) {
            continue; // loop to category selection
        } else {
            break;
        }
    }
